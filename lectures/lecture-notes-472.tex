...
\newpage
\section{2026-02-23 | Week 06 | Lecture 14}
\subsection{Confidence intervals}
\textit{Based on section 8.5 in the textbook.}
...
$[\widehat{\theta}_{L},\widehat{\theta}_{U}]$, where $\widehat{\theta}_{L}$ and
$\widehat{\theta}_{U}$ are a pair of random variables (which vary from sample
to sample).
\begin{remark}
  Desired properties:
  \begin{itemize}
...
probability that a (random) confidence interval will enclose the terget
...
Suppose $\widehat{\theta}_{L}$ and $\widehat{\theta}_{U}$ are the upper and
lower confidence limits for a parameter $\theta$. If
\begin{equation*}
...
\begin{example}
  ...
  \fl{Solution:} Recall that a random variable $X$ is an exponetial with mean
  $\mu$ if
  ...
  \textbf{Story:} Suppose you decide to spend your Saturday standing at an idyllic
  country road waiting for cars to pass, measuring the time between cars. We
  assume the inter-arrival times between cars is exponentially distributed.
  The unknown parameter $\theta$ is the average time between cars in, say,
  minutes. Suppose the time between the first two cars to pass is $Y=12$
  minutes. Then we compute $\widehat{\theta}_{L}= \frac{12}{2.996} \approx 4$
  and $\widehat{\theta}_{U}= \frac{9}{.51}\approx 24$. So with probability
  $90\%$, the next car will take between 4 and 24 minutes to arrive.
\end{example}


We've discussed \demph{two-sided confidence intervals}, which take the form
$[\widehat{\theta}_{L},\widehat{\theta}_{U}]$. A \demph{lower one-sided
  confidence interval} takes the form $[\hat{\theta}_{L},\infty)$, and an
\demph{upper} one-sided confidence interval takes the form
$(-\infty,\widehat{\theta}_{U}]$.
