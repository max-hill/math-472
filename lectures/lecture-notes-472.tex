...
\Fl{Question:} Suppose we wish to estimate a parameter $\theta$ with an
estimator $\widehat{\theta}$ based on a sample $X_{1},\ldots,X_{n}$. By it's
nature the statistic summarizes the sample $X_{1},\ldots,X_{n}$. Is any
information about $\theta$ lost? How can we know if the estimator uses all the
possible information available in the sample? Or can we learn more about the
target parameter by considering some other statistic of the sample?

Informally, a statistic that utilizes \textit{all} the information in a sample
about a target parameter is said to be a \textit{sufficient statistic}, which
we will define mathematically below.
