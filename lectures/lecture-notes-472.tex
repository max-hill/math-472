...
\begin{theorem}
  Let $X$ and $Y$ be indepndent random variables. Let $g(x)$ be a function
...
\begin{remark}
  ...
  \textit{asymptotically normally distributed with mean
    ...
    $\sigma^{2}_{\overline{X}}=\sigma^{2}/n$}.

\end{remark}

\begin{example}[Example 7.9 in the textbook]
  A Ukrainian munitions factory produces a 155mm artillery shell every 1.5
  minutes, with the production time having a variance of 1.
  ...
  \end{align*}
  Because the sample size is large, the central limit theorem tells us that
  $\overline{Y}$ is approximately normally distributed with mean
  $\mu_{\overline{Y}}=1.5$ and variance $\sigma^{2}_{\overline{Y}}=1/100$.
  Standardizing $\overline{Y}$, we have
  \begin{align*}
    \P\left[\overline{Y}\leq 1.2\right]
    &= \P\left[ \frac{\overline{Y}-1.5}{1/\sqrt{100}}\leq \frac{1.2-1.5}{1/\sqrt{100}}\right]\\
    &\approx \P\left[Z \leq -3\right]\\
    &=.0013.
  \end{align*}
...
\begin{example}[Example 7.10 in the textbook]
  ...
  of the city's voters favor her. If $n=100$ voters show up to vote at precint
  ...
  \begin{align*}
    ...
    &\approx \P\left[Z \geq \frac{.55-.5}{\sqrt{.0025}}\right]\\
    &\approx \P\left[Z \geq 1\right]\\
    &=.1587.
  \end{align*}
\end{example}
...
\begin{definition}[Biased and unbiased]
  ...
  $B(\widehat{\theta})= \E\left[\widehat{\theta} \right]-\theta$.
\end{definition}

\begin{remark}
  Since $\widehat{\theta}$ is a statistic, it is a function of a random
  sample. So when we write $\E\left[\widehat{\theta} \right]$, we really mean
...
\begin{example}
  ...
  Given $n$ rolls, the sample $n$ random variables
  \begin{equation*}
    X_{1},\ldots,X_{n}
  \end{equation*}
  ...
  \begin{itemize}
    ...
    $\E\left[\widehat{\theta} \right]<\theta$. This is because it will
    ``usually'' be less than the true number of sides, and will never be more.
    ...
    Let's demystify this estimator. We know that since the dice has $\theta$
    sides, it has expected value
    \begin{equation*}
    ...
    \begin{equation*}
      \frac{X_{1},\ldots,X_{n}}{n} \approx \E\left[X \right].
    \end{equation*}
    Combining this with \cref{eq:7} implies
    \begin{equation*}
      \theta 
    ...
    \begin{equation*}
      2 \left( \frac{3+11+7+4}{4} \right) = 2 \left( \frac{25}{4} \right) -1 = 11.5.
    \end{equation*}
...
\begin{definition}[Mean square error]
  ...
  The \demph{mean square error} (MSE) of a point estimator $\widehat{\theta}$ is
  \begin{equation*}
    \mathrm{MSE}(\widehat{\theta}) = \E\left[\left( \widehat{\theta}-\theta \right)  \right]
  \end{equation*}
\end{definition}

This is the average square of the distance between the estimator and the
target parameter. We want the MSE to be small.

The next theorem says that MSE is a combined measure of both the estimator's
variance and its bias.
\begin{theorem}
  Let $\widehat{\theta}$ be an estimator such that $\E\left[\widehat{\theta}
  \right]$ is defined. Then,
  \begin{equation*}
    \mathrm{MSE}(\widehat{\theta}) = \Var(\widehat{\theta}) + \mathrm{Bias}(\widehat{\theta}). 
  \end{equation*}
\end{theorem}
