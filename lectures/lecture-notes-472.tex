...
\begin{enumerate}
  ...
  Rao-Blackwell Theorem (9.5), MVUE (9.5), method of moments (9.6), method of
...
$X_{1},\ldots,X_{n}$. By it's nature the statistic summarizes the sample
...
\begin{example}[Sufficiency - motivating example]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{align*}
        ...
        &= \frac{p^{k}(1-p)^{n-k}}{{n\choose k} p^{k}(1-p)^{n-k}} = \frac{1}{{n\choose k}}
      \end{align*}
    \end{itemize}
  \end{itemize}
  ...
  known, no other function of $X_{1},\ldots,X_{n}$ will shed additional liight
...
If  $Y_{1},\ldots,Y_{n}$ is a random sample of discrete random variables, then the likelihood is
\begin{align*}
...
\begin{theorem}[Factorization criterion -- Theorem 9.4 in text]
  ...
  Let $T$ be a statistic based on the random sample $X=(X_{1},\ldots,X_{n})$,
  and let $x=(x_{1},\ldots,x_{n})\in \R^{n}$. Let $T:\R^{n}\to \R$ be
  function. Then $T(X)$ is a sufficient statistic for estimation of $\theta$
  if and only if we can factor the likelihood as follows:
  \begin{equation*}
    L(x\mid \theta) = g(T(x),\theta)\cdot h(x)
  \end{equation*}
...
\begin{example}
  ...
  \end{equation*}
  And therefore we conclude that $T(X)$ is a sufficient statistic for
  estimating $\theta$.
\end{example}
...
\begin{example}
  ...
  is 
  \begin{equation*}
    f(y_{i}\mid \theta) = \frac{1}{\theta}e^{-y_{i}/\theta}\mathbf{1}_{y_{i}\geq 0}.
  \end{equation*}
  for some parameter $\theta>0$. Show that $\overline{Y}$ is a suffficient
  statistic for $\theta$.

  \Fl{Solution:} Let $y=(y_{1},\ldots,y_{n})\in \R^{n}$
  \begin{align*}
    L(y\mid \theta)
    &= f(y_{1}\mid \theta)\cdots f(y_{n}\mid \theta)\\
    &= \frac{e^{-y_{1}/\theta}}{\theta}\times \frac{e^{-y_{2}/\theta}}{\theta}\times \cdots\times \frac{e^{-y_{n}/\theta}}{\theta}\\
    &= \frac{e^{-\overline{y}/\theta}}{\theta^{n}}
  \end{align*}
  again, $h(y)=1$, lol.
\end{example}

\begin{example}
  Let $\theta$ be a positive integer. Let $X_{1},\ldots,X_{n}$ an iid sample from the uniform distribution on
  $\left\{1,\ldots,\theta\right\}$ (aka each $X_{i}$ is a roll of a
  $\theta$-sided dice). Find a sufficient statistic for $\theta$.
  ...
  \end{equation*}
  The joint pdf of the sample $X_{1},\ldots,X_{n}$ is defined as follows:
  given for for each $x=(x_{1},\ldots,x_{n})\in \R^{n}$
  \begin{align*}
    p_{X}(x\mid \theta) 
    &= \P_{\theta}\left[X_{1}=x_{1},\ldots,X_{n}=x_{n} \right] \\
    &= \prod_{i=1}^{n}p(x_{i}\mid \theta)\\ 
