...
\begin{proof}
  \begin{align*}
    ...
    &\leq 1-\frac{1}{k^{2}} &&\text{by \cref{thm:chebychevs-inequality} }
  \end{align*}
\end{proof}
...
$2\sigma_{\hat{\theta}}$ with probability at least $.75$. This bound is
...
\begin{remark}
  ...
  establishing a $k$-standard-error bound, we want $\alpha$ to be as close to $1$ as
  possible.
\end{remark}
...
$2\sigma_{\hat{\theta}}$ with probability at least $\alpha=.75$. This bound is
conservative, since $\alpha=.75$ is not very close to 1.

Usually the true value of $\alpha$ is much higher. For example, $\alpha$ will
be around $.95$ when $\widehat{\theta}$ is approximately normally distributed
(e.g., when $\widehat{\theta}$ is a sample mean). We show that with the next
example.


\begin{example}[Example 8.2 in textbook]
  A sample of $n=1000$ voters randomly selected shoed $y=560$ in favor of
  ...
  \Fl{Solution:} Let $\hat{p}$ be the fraction of of the sample who support
  Jones. According to this estimator, our estimate is that $56\%$ of voters
  support Jones.

  How much faith should we put in this value?
  ...
  The question is asking us to approximate both $\sigma_{\hat{p}}$ and area of
  the grey shaded region.
  
  To do this, let's first compute the standard error of
  $\sigma_{\widehat{p}}$.
  ...
  Instead, we can approximate $\sigma_{\widehat{p}}$ with
  \begin{equation*}
  ...
  approximation, we can apply \cref{eq:two-standard-error-lower-bound}, which gives
  \begin{equation*}
    \P\left[\text{estimation error} \leq .03 \right] \approx \P\left[\left| \widehat{p}-p \right| \geq 2\sigma_{\widehat{p}}\right]  \geq .75.
  \end{equation*}
  Hence, the shaded region has area at least $.75$. But this is very conservative.

  Since we know that $\widehat{p}$ has an
  approximately Gaussian sampling distribution, we can do better than $.75$,
  as follows:

  \begin{align*}
    \P\left[ \text{estimation error} \leq .03\right] 
    &\approx \P\left[\left| \widehat{p}-p \right| \geq
      2\sigma_{\widehat{p}}\right] &&\text{since $\sigma_{\widehat{p}}\approx .015$ }\\
    &\approx \P\left[\left| \widehat{p}-p \right| \geq 2\sigma\right]
    &&\text{assuming $\sigma_{\widehat{p}}\approx \sigma$}\\
    &= \P\left[-2\leq  \frac{\widehat{p}-p}{\sigma} \leq 2\right] &&\text{standardizing}\\
    &\approx \P\left[ -2 \leq Z \leq 2\right] &&\text{by CLT}\\
    &=.95.
  \end{align*}
  Thus, if we were to repeat this poll 100 times, we would expect that only in
  5 such polls would the estimate deviate from the true $p$ by more than
  $.03$. So we can be reasonably confident that our estimate of $\widehat{p}=.56$ is within
  $.03$ of the true value of $p$.  
\end{example}

\section{2026-02-?? | Week ?? | Lecture ??}
...
A confidence interval varys from sample to sample.
