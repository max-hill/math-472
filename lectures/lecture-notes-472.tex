...
\begin{example}[Sum 2d4]
  \label{ex:sum-2d4}
  Roll a 4-sided dice twice (this is the \textit{experiment}). There are 16 possible
  \textit{outcomes}. The \textit{sample space} is
  \begin{equation*}
  ...
  \end{equation*}
  \textit{Events} are often defined using preimages of random variables. For
...
\begin{theorem}[Law of the unconscious statistician]
  ...
  \begin{enumerate}[label=\rm{(\roman*.)}]
    ...
    \begin{equation*}
      \E\left[g(X) \right]= \sum_{\rm{all}\ x} g(x)p(x).
    \end{equation*}
...
\begin{theorem}[Factorization theorem -- Theorem 5.4 in textbook]
  ...
  factorizability of the joint pdf/pmf. More formally, we have:
