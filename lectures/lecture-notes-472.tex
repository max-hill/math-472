...
\begin{remark}
  ...
  $\E\left[\hat{\theta}(X_{1},\ldots,X_{n}) \right]$, where
...
\begin{proof}
  ...
  $\E\left[\widehat{\theta}- \E[ \hat{\theta}] \right]= \E[\widehat{\theta} ]-
  \E[\widehat{\theta}]$.
\end{proof}
...
$\epsilon=|\hat{\theta}-\theta|$ is also a random quantity, so we cannot say
...
$[\widehat{\theta}_{L},\widehat{\theta}_{U}]$. A \demph{lower 1-sided
  confidence interval} takes the form $[\hat{\theta}_{L},\infty)$, and an
...
\begin{example}[This is like Example 8.9 in the textbook]
  ...
  $\hat{p} \to p$ as $n$ grows, which implies
  \begin{equation*}
  ...
  \begin{equation*}
    n \geq  1600p(1-p).
  \end{equation*}
  Oops! We don't know $p$. But there's a fix. Observe that $p(1-p) \leq 1/4$
  for any value of $p$. Therefore, to be conservative, it is sufficient for
  \begin{equation*}
    n \geq 1600\cdot \frac{1}{4} = 400
  \end{equation*}
  since this implies \cref{eq:16} holds regardless of the value of $p$.
  Therefore the pollster needs to poll at least 400 people.
