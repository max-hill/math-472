...
\begin{problem}[Exercise 9.8]
  ...
  \end{equation*}
  (Note that in the definition of $\mathcal{I}(\theta)$, the $Y$ is a random
  variable with density function $f$.) If
...
\begin{problem}[Exercise 9.3 and 9.15?]
  Let $Y_{1},\ldots,Y_{n}$ denote a random sample from the uniform
  distribution on the interval $(\theta,\theta+1)$. Let
  \begin{equation*}
    \widehat{\theta}_{1} 
    = \overline{Y}-\frac{1}{2} \quad \text{cand} \quad \widehat{\theta}_{2} 
...
\begin{problem}[Optional]
  Make a math meme related to statistics.
\end{problem}
