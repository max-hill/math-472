...
\begin{problem}
  A discrete random variable $X$ is said to be \demph{binomially distributed}
  with parameters $n$ (\demph{number of trials}) and $p$ (the \demph{success
    probability}) if it has probability mass fucntion
  \begin{equation*}
    f_{X}(k) = {n\choose k} p^{k}(1-p)^{n-k}
  \end{equation*}
  for $k\in \left\{0,1,2,\ldots,n\right\}.$
  Fix $\lambda>0$. Suppose that $p=\lambda/n$. Show that
  \begin{equation*}
    \lim_{n\to\infty} f_{X}(k) = \frac{\lambda^{k}}{k!}e^{-\lambda}
  \end{equation*}
  This shows that, in some sense, the Poisson distribution is the limit of the
...
\begin{problem}
  Prove that if $P(\cdot)$ is a probability function and that $B$ is an event
  with $P(B)>0$, then the function $P(\cdot \mid B)$ is also a probability
  function.
\end{problem}
