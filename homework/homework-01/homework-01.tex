...
memorized (they are fair game for in-class quizzes). Text in \textbf{bold}
should be reviewed if you are not familiar with it.
...
\begin{problem}
  The \demph{power set} of $\R$, denoted $2^{\R}$ is the set
  \begin{equation*}
    2^{\R} := \left\{A: A\subseteq \R\right\}. 
  \end{equation*}
  Let $P:2^{\R} \to \left\{0,1\right\}$ be the function defined by
  \begin{equation*}
    P(A) := \left\{ \begin{array}{l@{\quad:\quad}l} 1 & A\text{ is
          uncountable}\\ 0& \text{ otherwise} \end{array}\right.
  \end{equation*}
  Verify that $P$ is a probability measure on $2^{\R}$.
\end{problem}
...
\begin{problem}[The Law of Rare Events]
  ...
  \begin{enumerate}[(a)]
    ...
    \end{equation*}
    \item Part \ref{item:5} shows that if $n$ is large and $p$ is small (and
    $\lambda=np$ isn't too big), then a random variable $X\sim
    \text{Bin}(n,p)$ is well-approximated by the distribution Pois$(np)$.
...
\begin{problem}[This is like Example 2.1 in the textbook]
...
\begin{problem}
  Prove proposition 2 in the lecture notes.
  
  \textit{If you get stuck, you can find this proof in my old math 372 lecture
    notes (Week 2, Class 4, available on the course website).}
\end{problem}
% \begin{problem}
%   \begin{enumerate}[(a)]
...
\begin{problem}
  ...
  \begin{equation*}
    f_{X}(x) = \frac{1}{\pi} \frac{1}{1+x^{2}}, \ x\in \R.
  \end{equation*}
  \begin{enumerate}[(a)]
    \item Show that $f_{X}$ is a probability density function.
...
\begin{problem}[Exercise 2.86 in the textbook]
  Suppose that $A$ and $B$ are two events such that $\P\left[A \right]=.8$
  and $\P\left[B \right]=.7$.
  \begin{enumerate}[(a)]
...
\begin{problem}[Exercise 2.73 in the textbook]
  Gregor Mendel was a monk who, in 1865, suggested a theory of inheritance based
  on the science of genetics. He identiﬁed heterozygous individuals for ﬂower
  color that had two alleles (one r = recessive white color allele and one R =
  dominant red color allele). When these individuals were mated, 3/4 of the
  offspring were observed to have red ﬂowers, and 1/4 had white ﬂowers. The
  following table summarizes this mating; each parent gives one of its alleles
  to form the gene of the offspring.
 
  \begin{equation*}
    \begin{array}{c|cc}
      & \multicolumn{2}{c}{\textbf{Parent 1}} \\[-0.5ex]
      \textbf{Parent 2} & r & R \\ \hline
      r & rr & rR \\
      R & rR & RR
    \end{array}    
  \end{equation*}
  We assume that each parent is equally likely to give either of the two
  alleles. We also assume that if either one or two of the alleles in a pair is
  dominant (R), the offspring will have red ﬂowers. What is the probability that
  an offspring has
  \begin{enumerate}[(a)]
    \item at least one dominant allele?
    \item at least one recessive allele?
    \item one recessive allele, given that the offspring has red ﬂowers?
  \end{enumerate}
\end{problem}


\begin{problem}
  A \demph{median} of a distribution is a value $m$ such that $\P\left[X \leq
