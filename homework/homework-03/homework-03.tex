...
\begin{problem}[Binomial distribution, revisited]
  ...
  \end{equation*}
  where the random variables $\xi_{1},\ldots,\xi_{n}$ are independent with
  \begin{equation*}
...
\begin{problem}
  ...
  notes). The amount of ﬁll dispensed by a bottling machine is normally
...
\begin{problem}
  ...
  forester samples $n = 9$ trees, ﬁnd the probability that the sample mean will
...
\begin{problem}[Refer to \cref{problem:2}] Suppose that in the forest
  ...
  basal areas is to be measured, ﬁnd a statistic $A$ such that
  \begin{equation*}
