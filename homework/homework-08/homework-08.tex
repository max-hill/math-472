...
\begin{problem}[Salk Polio Field Trial (1954)]
  ...
  % \begin{figure}
  %   \centering
  %   \includegraphics[scale=.3]{images/salk}
  %   \caption{Some participants in the 1945 Salk polio vaccine trial conducted
  %     by the National Foundation for Infantile Paralysis. See
  %     \url{https://www.randomservices.org/random/data/Polio.html}.}
  %   \label{fig:salk-picture}
  % \end{figure}
\end{problem}
...
\begin{problem}[Exercise 9.82]
  ...
  by
  \begin{equation*}
    f(y\mid \theta) = \left\{ \begin{array}{l@{\quad:\quad}l} \left( \frac{1}{\theta} \right)r y^{r-1}e^{-y^{r-1}}e^{-y^{r}/\theta}  & \theta>0, y>0\\ 0& \text{elsewhere.}\end{array}\right.
  \end{equation*}
...
\begin{problem}
  ...
  \end{equation*}
  \begin{enumerate}[(a)]
    \item Find a suffificent statistics for $\theta$. 
    \item Find the MLE of $\theta$. 
  \end{enumerate}
\end{problem}
...
\begin{problem}[Exercise 10.19]
  ...
  The output voltage for an electric circuit is speciﬁed to be 130. A sample
...
\begin{problem}[Exercise 10.37]
  ...
  $H_{a}:\mu=128$, ﬁnd the probability of a type II error, $\beta$ for the
